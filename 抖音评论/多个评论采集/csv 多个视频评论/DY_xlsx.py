...
class Xlsx():  # 读取xlsx 文件中https://v.douyin.com/iKeD74w/链接
    ...
    def def_csv(self, ):  # 保存数据csv文件
        for data in self.xlsx():
            ...
            if not os.path.exists('评论链接.csv'):
                dataframe.to_csv('./评论链接.csv', mode='a', encoding='utf-8-sig', index=False, header=False, )
            dataframe.to_csv('./评论链接.csv', mode='a', encoding='utf-8-sig', header=False, index=False)
            print('Link saved!')
