...
class Xlsx():  # 读取xlsx 文件中https://v.douyin.com/iKeD74w/链接
    ...
    def def_csv(self, ):  # 保存数据csv文件
        for data in self.xlsx():
            ...
            if not os.path.exists('点赞和评论url.csv'):
                dataframe.to_csv('./点赞和评论url.csv', mode='a', encoding='utf-8-sig', index=False, )
            dataframe.to_csv('./点赞和评论url.csv', mode='a', encoding='utf-8-sig', header=False, index=False)

            df = pd.read_csv('点赞和评论url.csv')
            # 删除重复项
            df_s = df.drop_duplicates()
            # 保存修改后的DataFrame对象到新的CSV文件
            df_s.to_csv('点赞和评论url.csv', index=False)
        print('Link saved!')
