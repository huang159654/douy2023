...
class Xlsx():  # 读取xlsx 文件中https://v.douyin.com/iKeD74w/链接
    ...
    def def_csv(self, ):  # 保存数据csv文件
        for data in self.xlsx():
            dataframe = pd.DataFrame(data=data)
            if not os.path.exists(r'C:\Users\Administrator\PycharmProjects\pythonProject\函数\抖音\抖音js\ALL_csv\url.csv'):
                dataframe.to_csv(r'C:\Users\Administrator\PycharmProjects\pythonProject\函数\抖音\抖音js\ALL_csv\url.csv', mode='a', encoding='utf-8-sig', index=False, )
            dataframe.to_csv(r'C:\Users\Administrator\PycharmProjects\pythonProject\函数\抖音\抖音js\ALL_csv\url.csv', mode='a', encoding='utf-8-sig', header=False, index=False)
            df = pd.read_csv(r'C:\Users\Administrator\PycharmProjects\pythonProject\函数\抖音\抖音js\ALL_csv\url.csv')
            # 删除重复项
            df_s = df.drop_duplicates()
            # 保存修改后的DataFrame对象到新的CSV文件
            df_s.to_csv(r'C:\Users\Administrator\PycharmProjects\pythonProject\函数\抖音\抖音js\ALL_csv\url.csv', index=False,)
        print('Link saved!')
...
if __name__ == '__main__':
    # Xlsx().def_csv()#保存csv文件 https://v.douyin.com/iKeD74w/ 需要转换
    Xlsx_video().def_csv()#保存csv文件 https://www.douyin.com/video/7312743380214238498
