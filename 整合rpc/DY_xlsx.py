...
class Xlsx():  # 读取xlsx 文件中https://v.douyin.com/iKeD74w/链接
    ...
    def xlsx(self):
        df = pd.read_excel('抖音推广链接.xlsx', sheet_name='Sheet1', )
        ...
        for i in data:  # 使用pandas转换xlsx数据
            try:
                ...
                with open('错误链接.txt', mode='a', ) as f:
                    txt = value + '\n'  # 数据分行
                    f.write(txt)
    ...
    def def_csv(self, ):  # 保存数据csv文件
        for data in self.xlsx():
            id = data['id']
            dataframe = pd.DataFrame(data=id)
            if not os.path.exists('101190400.csv'):
                dataframe.to_csv('./101190400.csv', mode='a', encoding='utf-8-sig', index=False, header=False, )
            dataframe.to_csv('./101190400.csv', mode='a', encoding='utf-8-sig', header=False, index=False)
            print(id, '保存完毕！')

    def def_sql_url(self):  # 保存到数据库
        db = pymysql.connect(host='localhost', user='root', password='huang159', port=3306, db='dy_creat')
        mycursor = db.cursor()
        sql = "insert into url (dy_url) VALUES (%s);"
        for data in self.xlsx():
            try:
                values = (data["id"][0])
                ...
                with open('url错误链接.txt', mode='a', ) as f:
                    txt = data["id"][0] + '\n'  # 数据分行
                    f.write(txt)
...
class Xlsx_video():  # 读取xlsx 文件中https://www.douyin.com/video/7312743380214238498 链接
    ...
    def def_xlsx(self):
        df = pd.read_excel('抖音推广链接.xlsx', sheet_name='Sheet2', )
        data = df.values.tolist()
        for i in data:  # 使用pandas转换xlsx数据
            try:
                value = i[3]
