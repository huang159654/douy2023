...
class Sql_data():
    ...
    def sql_table(self):
        ...
        cursor = db.cursor()
        sql = 'CREATE TABLE IF NOT EXISTS comments (id INTEGER NOT NULL AUTO_INCREMENT,awem_id VARCHAR(400) NOT NULL, cid VARCHAR(400) NOT NULL, text_s TEXT,digg_count TEXT,reply_comment_total TEXT,ip_label TEXT,create_time TEXT,nic_kname TEXT, PRIMARY KEY (id))'
        cursor.execute(sql)
        db = pymysql.connect(host='localhost', user='root', password='huang159', port=3306, db='dy_creat')
        cursor = db.cursor()
